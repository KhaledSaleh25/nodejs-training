...
people.shift// 1) Remove the person at the FRONT who decided to leave.

people.unshift("luna")// 2) Add "Luna" to the VERY FRONT.

let lasttwo =people.splice(-2)// 3) Move the LAST TWO people to be right AFTER the first element (keep their order).
...
// Use &&, ||, and !
let isNiceNumber = ((a%2===0)&& (b%2===1))||!c;
...
if(!values[0]) falsy.push(values[0])
    else truthy.push(values[0])
if(!values[1]) falsy.push(values[1])
    else truthy.push(values[1])
if(!values[2]) falsy.push(values[2])
    else truthy.push(values[2])
if(!values[3]) falsy.push(values[3])
    else truthy.push(values[3])
if(!values[4]) falsy.push(values[4])
...
let displayName=trimmed.charAt(0).toUpperCase()+trimmed.slice(1).toLowerCase();
